...
async def run():
    llm = LLMClient(        base_url=load_config().model.base_url,
                    api_key=load_config().model.api_key,
                            default_model=load_config().model.model_name)
    logger.info(f"LLM {load_config().model.base_url},{load_config().model.model_name}")
    mcp_client = MCPClient(llm=llm)
    await mcp_client.connect_to_server_by_sse("http://127.0.0.1:8090/sse")
    await mcp_client.get_server_resources()
