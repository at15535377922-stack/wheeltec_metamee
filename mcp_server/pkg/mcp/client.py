...
from pkg.mcp.llm import LLMOpenAICompti
...
class MCPClient:
    ...
    async def process_query_by_sse_stream(self, query, resource_uris=None):
        ...
        chat_messages = {"messages": messages, "stream": True}
        ...
        while curr_round <= CHAT_MAX_ROUND and continue_loop:
            response = await self.llm.chat_completions(chat_messages)
            if not response:
                continue
...
async def main():
    ...
    try:
        await client.connect_to_server_by_sse("http://10.10.185.9:8090/sse")
