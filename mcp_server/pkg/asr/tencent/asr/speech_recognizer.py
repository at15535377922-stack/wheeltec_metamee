...
import uuid
import urllib
from pkg.asr.tencent.common.log import logger
...
class SpeechRecognizer:
    ...
    def start(self):
        ...
        def on_close(ws, arg1, arg2):
            self.status = CLOSED
            logger.info("websocket closed  voice id %s" % self.voice_id)
    ...
    def wait_connection(self):
        start = time.time()
        while (
            self.status != OPENED and self.status != ERROR and time.time() - start < 60
        ):
            time.sleep(0.5)
