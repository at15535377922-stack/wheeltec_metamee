...
async def run():
    ...
    threading.Thread(target=speaker.play, daemon=True).start()
            ...
            try:
                if data is not None:
                    ...
                    if is_running:
                        ...
                        asr_result_queue.put(asr_result)
    ...
    def tts_to_player_job():
        async def tts_to_player():
            async for audio_chunk in tts_client.get_audio_buffer():
                speaker.append_audio(audio_chunk)

        asyncio.run(tts_to_player())

    threading.Thread(target=tts_to_player_job).start()

    loop = asyncio.get_event_loop()
    while True:
        asr_result = asr_result_queue.get()
